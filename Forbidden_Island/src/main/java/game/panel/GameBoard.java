...
public class GameBoard extends JPanel implements MouseListener {
    ...
    private void handleTurnEnd() {
        ...
        // --- TREASURE CARD DRAWING AND PROCESSING ---
        System.out.println("DEBUG: Starting treasure card draw phase for " + playerWhoseTurnEnded.getRole() + ".");
        for (int i = 0; i < 2; i++) { // Players draw 2 treasure cards
            String drawnCardName = gameState.drawTreasureCard();
            if (drawnCardName != null) {
                if (drawnCardName.equals("WatersRise")) {
                    gameState.handleWatersRiseEffect();
                    gameState.discardTreasureCardToPile(drawnCardName); // Waters Rise cards are discarded immediately
                    System.out.println("DEBUG: " + playerWhoseTurnEnded.getRole() + " drew WATERS RISE! Water level increased.");
                    updateWaterLevelMeterDisplay(); // Add this line to refresh water level UI
                } else {
                    playerWhoseTurnEnded.drawCard(drawnCardName);
                    // TODO: Handle hand limit if necessary, as per game rules (usually 5 cards).
                    // This is usually done immediately after drawing/receiving cards.
                    // The Player.getHandSize() and discard logic are available.
                    // if (playerWhoseTurnEnded.getHandSize() > 5) { /* prompt discard */ }
                }
            } else {
                System.out.println("DEBUG: No more treasure cards to draw.");
                break; // No more cards in deck or discard
            }
        }
