...
public class GameState {
    ...
    /**
     * Checks if all players are unable to move or act.
     * This is a complex condition that might involve checking if all accessible tiles are sunk
     * or if there are no valid moves/actions left for any player.
     * @return true if all players are stuck, false otherwise.
     */
    public boolean areAllPlayersStuck() {
        for (Player player : allPlayers) {
            // A player is not stuck if they can move to any valid tile
            // For simplicity, we define "stuck" as being unable to move to any un-sunk, non-fools-landing tile
            // More complex logic might involve checking for available actions (e.g., shore up, give card)
            // For now, let's focus on movement.
            if (!findMovable(player).isEmpty()) {
                return false; // At least one player can move, so not all players are stuck
            }
        }
        return true; // All players are stuck (cannot move)
    }

    /**
     * Checks if all players are currently on sunk tiles.
     * This is a direct loss condition.
     * @return true if all players are on sunk tiles, false otherwise.
     */
    public boolean areAllPlayersOnSunkTiles() {
        for (Player player : allPlayers) {
            GameTile playerTile = getTileAtPosition(player.getPos());
            if (playerTile == null || !playerTile.isSunk()) {
                return false; // At least one player is not on a sunk tile
            }
        }
        return true; // All players are on sunk tiles
    }

    /**
     * Helper method to get a GameTile by its coordinates.
     * @param position The [row, col] coordinates of the tile.
     * @return The GameTile at the specified position, or null if out of bounds or no tile exists.
     */
    public GameTile getTileAtPosition(int[] position) {
    ...
    public boolean checkOverallLossCondition() {
        ...
        if (areAllPlayersStuck()) {
            System.out.println("GAME OVER: All players are unable to move or act!");
            return true;
        }
        if (areAllPlayersOnSunkTiles()) {
            System.out.println("GAME OVER: All players are on sunk tiles!");
            return true;
        }
        return false;
    }
